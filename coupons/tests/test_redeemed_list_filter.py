...
class RedeemedListFilterTests(BasicTest):
    ...
    def test_can_list_redeemed_filtering(self):
        """
        ...
        with self.settings(ROOT_URLCONF='coupons.urls'):
            ...
            response = self.client.get('/redeemed', {'user': self.user.id}, format='json')
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assertEqual(1, len(response.data))
            self.assertEqual(self.user.id, response.data[0]['user'])

            response = self.client.get('/redeemed', {'user': self.user2.id}, format='json')
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assertEqual(1, len(response.data))
    ...
    def test_cant_list_redeemed_filtering(self):
        """
